class CellStageScene extends Phaser.Scene {
  ...
  create() {
    this.worldWidth = 2520;
    this.worldHeight = 1580;
  ...
  buildPartMenu() {
    const definitions = [
      { key: 'filterMouth', label: 'Filter Mouth', desc: 'Herbivore feeding; eat plant matter.', unlock: 'Available at start.' },
      { key: 'jaw', label: 'Jaw', desc: 'Eat meat chunks and damage enemies with bites.', unlock: 'Unlock from meteor shards.' },
      { key: 'proboscis', label: 'Proboscis', desc: 'Omnivore feeding and slow drain attacks.', unlock: 'Unlock from meteor shards.' },
      { key: 'spike', label: 'Spike', desc: 'Stab enemies and trigger meat chunk drops.', unlock: 'Unlock from meteor shards.' },
      { key: 'cilia', label: 'Cilia', desc: 'Faster turning and improved control.', unlock: 'Unlock from meteor shards.' },
      { key: 'electric', label: 'Electric', desc: 'Shock enemies for extra damage.', unlock: 'Unlock from meteor shards.' },
      { key: 'poison', label: 'Poison', desc: 'Area denial and immunity to poison clouds.', unlock: 'Unlock from meteor shards.' },
    ];
  ...
  spawnEnemies(amount) {
    for (let i = 0; i < amount; i++) {
      const x = Phaser.Math.Between(120, this.worldWidth - 120);
      const y = Phaser.Math.Between(120, this.worldHeight - 120);
      const enemy = this.add.circle(x, y, Phaser.Math.Between(18, 34), 0xd250f5, 0.92).setDepth(1.4);
      enemy.foodType = 'enemy';
      enemy.sizeCategory = Phaser.Math.Between(1, 3);
      enemy.health = 2 + enemy.sizeCategory * 2;
      enemy.attack = 1 + enemy.sizeCategory;
      enemy.speed = 20 + enemy.sizeCategory * 18;
  ...
  nextUnlockPart() {
    const order = ['jaw', 'proboscis', 'spike', 'cilia', 'electric', 'poison'];
